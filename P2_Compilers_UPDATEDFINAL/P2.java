...
public class P2 {
	...
	private static void testCharLine() throws IOException {

		// Declares a new instance of the FileReader type called Filein to read
		// character files
		FileReader inputFile = null;
		// Declares a new instance of the PrintWriter type called outputFile to print
		// the output
		PrintWriter outputFile = null;

		// Must put file reader in try/catch to adhere for instance when the file is not
		// found
		try {
		...
		while (token.sym != sym.EOF) {
			/*
			 * This writes to the output file and writes the character number of the current
			 * token, the line number of this current token and the symbol that it
			 * corresponds to
			 */
			outputFile.println(((TokenVal) token.value).charnum + "(" + ((TokenVal) token.value).linenum + ")  sym # : "
					+ token.sym);
	 ...
	 * 
	 */
	private static void testEndOfFile() throws IOException {
	...
	/*
	 * test string lit test int lit test id test charnum and yyline EOF
	 * 
	 */
